...
class TestAuth(unittest.TestCase):
    ...
    def register(self, firstname: str = "test", lastname: str = "test",
                 email: str = "test@gmail.com", password: str = "Pas$word123") -> Response:
    ...
    def login(self, email: str = "test@gmail.com", password: str = "Pas$word123") -> Response:
        return self.client.post("/login", json={
            "email": email,
            "password": password
        })

    def check_refresh_cookie(self, response: Response):
        cookie = response.headers["Set-Cookie"]
        path = list(filter(lambda x: x.strip().startswith("Path"), cookie.split(";")))[0]
    ...
    def test_whoami(self):
        response = self.register()
        access_token = response.get_json()["access_token"]
        response = self.client.get("/whoami", headers={"Authorization": f"Bearer {access_token}"})
        self.assertEqual(200, response.status_code)
        json = response.get_json()
    ...
    def test_activate(self):
        with self.app.app_context():
            self.register()
            user: User = db.session.query(User).filter_by(email="test@gmail.com").first()
            ...
            response = self.client.get(f"/activate/123")
            self.assertEqual(404, response.status_code)
    ...
    def test_send_activation_link(self):
        with self.app.app_context():
            response = self.register()
            access_token = response.get_json()["access_token"]
            response = self.client.post("/send-activation-link",
    ...
    def test_get_all_devices(self):
        ...
        response = self.login()
        access_token = response.get_json()["access_token"]
        response = self.client.get("/devices", headers={"Authorization": f"Bearer {access_token}"})
        self.assertEqual(200, response.status_code)
        json = response.get_json()
    ...
    def test_logout_all_devices(self):
        ...
        response = self.login()
        access_token = response.get_json()["access_token"]
        response = self.client.delete("/devices", headers={"Authorization": f"Bearer {access_token}"})
        self.assertEqual(200, response.status_code)
    ...
    def test_logout_device(self):
        ...
        refresh_token2 = response.headers["Set-Cookie"].split("=")[1].split(";")[0]
        response = self.client.delete("/devices/1", headers={"Authorization": f"Bearer {access_token2}"},
                                      json={"password": "Pas$word123"})
        self.assertEqual(204, response.status_code)
        # check if 1st device is logged out
        response = self.client.get("/refresh", headers={"Cookie": f"refresh_token={refresh_token}"})
        self.assertEqual(401, response.status_code)
        response = self.client.get("/whoami", headers={"Authorization": f"Bearer {access_token}"})
        self.assertEqual(401, response.status_code)
        # check if 2nd device is still logged in
        response = self.client.get("/whoami", headers={"Authorization": f"Bearer {access_token2}"})
        self.assertEqual(200, response.status_code)
        response = self.client.delete("/devices/1", json={"password": "Pas$word123"})  # no credentials
        self.assertEqual(401, response.status_code)
